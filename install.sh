...
bundle=b''.join((manifest_path.parent/item['name']).read_bytes() for item in meta['parts'])
expected=meta['files']; candidates={name:[] for name in expected}; pos=0; prefix=b'<<<GLACIEREQ_FILE '
while pos < len(bundle):
    ...
    if rel in expected and hashlib.sha256(payload).hexdigest()==expected[rel]: candidates[rel].append(payload)
    pos=start+size
found={}
for rel,digest in expected.items():
    ...
    target=dest/rel; target.parent.mkdir(parents=True,exist_ok=True); target.write_bytes(matches[0]); found[rel]=digest
projection={'schema':'glaciereq.runtime-projection.v2','canonical_repository':meta['canonical_repository'],'canonical_commit':commit,'files':expected,'generated_at':datetime.now(timezone.utc).isoformat(),'projection_law':'Only the unique payload matching each pinned canonical SHA-256 is materialized; nonmatching historical transport records have no authority.'}
