...
NOTION_TOKEN = os.environ.get("NOTION_API_KEY", "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")
...
def test_connection():
    ...
    resp = requests.get("https://api.notion.com/v1/users/me", headers=get_headers(), timeout=10)
...
def list_databases():
    ...
    resp = requests.post("https://api.notion.com/v1/search", headers=get_headers(), json={
        "filter": {"value": "database", "property": "object"},
        "page_size": 100
    })
    if resp.status_code == 200:
        dbs = resp.json().get("results", [])
        return [{"id": db["id"], "title": db.get("title", [{}])[0].get("plain_text", "Untitled")} for db in dbs]
    return []

def create_page(database_id, properties):
    """Create a page in a Notion database."""
    resp = requests.post("https://api.notion.com/v1/pages", headers=get_headers(), json={
...
def sync_case_to_notion(case_dir):
    ...
    for f in Path(case_dir).rglob("*.md"):
        props = {
            ...
            "File Path": {"rich_text": [{"text": {"content": str(f.relative_to(case_dir))[:2000]}}]},
        }
...
def categorize(filename):
    ...
    if "MOTION" in upper: return "Motion"
    if "EVIDENCE" in upper: return "Evidence"
    if "FORENSIC" in upper: return "Forensic"
    if "DEFENDANT" in upper or "SMASH" in upper: return "Defendant"
    if "TIMELINE" in upper: return "Timeline"
    if "DAMAGE" in upper: return "Damages"
    return "Filing"
