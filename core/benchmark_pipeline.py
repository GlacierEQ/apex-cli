...
# Load configurations
NEON_CONN_STRING = (
    os.environ.get("APEX_NEO4J_DIRECT_CONN")
    or "postgresql://glaciereq-owner@c-6.us-east-1.aws.neon.tech/1009?sslmode=require"
)
...
async def test_route_latency(session, connector: str) -> float:
    payload = {
        "connector": connector,
        "payload": {"jsonrpc": "2.0", "method": "tools/list", "params": {}, "id": 999},
    }
...
async def run_load_test(concurrency: int = 10):
    async with aiohttp.ClientSession() as session:
        ...
        p95_latency = (
            sorted(valid_results)[int(len(valid_results) * 0.95)]
            if valid_results
            else 0.0
        )
...
def log_metrics_to_neon(avg_ms: float, p95_ms: float, success_rate: float):
    ...
    try:
        ...
        conn.close()
        print(
            f"💾 Metrics written to Neon 1009: {avg_ms:.2f}ms avg / {p95_ms:.2f}ms p95"
        )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(
        f"Metrics: Average: {metrics['avg_ms']:.2f}ms | P95: {metrics['p95_ms']:.2f}ms | Success: {metrics['success_rate']:.1f}%"
    )
