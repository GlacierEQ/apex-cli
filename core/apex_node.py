...
try:
    ...
    HAS_OCR = False
    print("WARNING: OCR dependencies not installed. Run: uv pip install pdfplumber pytesseract pdf2image")
...
def generate_manifest_payload(file_path: Path, extracted_text: str) -> tuple:
    ...
    new_filename = f"{CASE_NUMBER}_{date_str}_INT_v1_RawIntake_{file_hash[:8]}{file_path.suffix}"
...
if HAS_MCP:
    ...
    def read_recent_manifests(limit: int = 5) -> list:
        manifests = []
        json_files = sorted(SUBSTRATE_DIR.glob("*.json"), key=os.path.getmtime, reverse=True)
    ...
    def list_evidence() -> list:
        ...
        for f in SUBSTRATE_DIR.iterdir():
            if f.suffix == '.json':
                with open(f) as fh:
                    data = json.load(fh)
                    files.append({"filename": f.stem, "sha256": data.get("sha256_checksum", "")[:16], "status": data.get("status", "")})
        return files
