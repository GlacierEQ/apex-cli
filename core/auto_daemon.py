...
def check_space():
    ...
    try:
        ...
        if free_gb < 5:
            ...
            subprocess.run(
                ["python3", str(HOME / "scripts" / "space_daemon.py")], timeout=30
            )
...
def push_to_github():
    ...
    try:
        ...
        if result.stdout.strip():
            subprocess.run(
                ["git", "-C", str(CYBERTACK / "LEAN_CASE"), "add", "-A"], timeout=10
            )
            subprocess.run(
                [
                    "git",
                    "-C",
                    str(CYBERTACK / "LEAN_CASE"),
                    "commit",
                    "-m",
                    f"Auto-sync: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                ],
                capture_output=True,
                timeout=10,
            )
