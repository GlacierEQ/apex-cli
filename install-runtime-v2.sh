...
PY
cp "$ADAPTER_DIR/runtime_cli.py" "$PROJECTION_DIR/scripts/runtime_cli.py"; cp "$ADAPTER_DIR/verify_projection.py" "$PROJECTION_DIR/scripts/verify_runtime_projection.py"; chmod +x "$PROJECTION_DIR/scripts/"*.py
...
HOME="$HOME_DIR" "$BIN_DIR/apex" help >/dev/null; HOME="$HOME_DIR" "$BIN_DIR/apex" boot "install verification" >/dev/null; HOME="$HOME_DIR" "$BIN_DIR/apex" runtime-status >/dev/null; HOME="$HOME_DIR" "$BIN_DIR/apex" replay "$CONFIG_DIR/last_runtime_receipt.json" >/dev/null
