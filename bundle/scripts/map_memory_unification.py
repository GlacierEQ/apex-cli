...
with open(doc_path, 'w') as f:
    ...
    f.write('This document maps the existing APEX Vault keys to the standardized Unified Memory Connect interface.\n\n')
    ...
    f.write('1. **Aliasing**: Standardize these variable names in the `~/.apex_vault/AGENTS/MASTER.env` for cross-agent compatibility.\n')
    f.write('2. **Connector Deployment**: Launch the `unified_memory_mcp.py` server to bridge these layers.\n')
    f.write('3. **Consistency Check**: Run `verify_mem0_layers.py` to ensure all 6 layers are responsive.\n')
