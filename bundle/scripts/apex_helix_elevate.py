...
def omega_elevate(start_daemon: bool) -> dict:
    ...
    if already_running:
        ...
        subprocess.Popen(
            ...
            cwd=str(OMEGA), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
...
Routine reversible work proceeds under active Operator mission authority. Validation and receipts prove the action; they do not become a serial approval queue.
...
def main() -> int:
    ...
    payload = {
        "at": _now(), "profile": "highest_form", "alpha": alpha, "omega": omega,
