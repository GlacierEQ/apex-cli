...
def test_installer_places_canonical_dispatcher(tmp_path: Path) -> None:
    env, installed = installed_apex(tmp_path)
    assert installed.is_file() and os.access(installed, os.X_OK)
    help_result = subprocess.run(
        [str(installed), "help"], env=env, text=True, capture_output=True, check=False
    )
...
def test_canonical_semantics_bind_payloads_not_fragment_containers() -> None:
    ...
    assert all(re.fullmatch(r"[0-9a-f]{64}", d) for d in manifest["files"].values())
    assert all("git_blob_sha1" not in p for p in manifest["parts"])
    names = [p["name"] for p in manifest["parts"]]
    assert len(names) == len(set(names))
    assert names[:5] == [f"canonical_part_{i:02d}.txt" for i in range(5)]
    assert names[5:] == ["source_part_06.txt", "source_part_07.txt"]
    assert "SHA-256" in manifest["law"]
    assert "zero or multiple hash-matching candidates fail closed" in manifest["law"]
...
def test_paths_module_honors_explicit_environment_root(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            "-c",
            "from core import paths; print(paths.BASE_DIR); print(paths.TASKLET_DIR); print(paths.WORKSPACE_DIR); print(paths.TMP_DIR); print(paths.CONNECTIONS_FILE)",
        ],
        cwd=ROOT,
        env=env,
        text=True,
...
def test_ready_finalize_requires_same_run_attested_receipt(tmp_path: Path) -> None:
    ...
    start = subprocess.run(
        [str(apex), "runtimectl", "start", "guard proof", "--run-id", "unattested-run"],
        env=env,
        text=True,
        capture_output=True,
        check=False,
    ...
    finalize = subprocess.run(
        [str(apex), "runtimectl", "finalize", "unattested-run", "READY"],
        env=env,
        text=True,
        capture_output=True,
        check=False,
    ...
    assert (
        "READY requires a persisted READY receipt for the same run" in finalize.stderr
    )
...
def test_concurrent_action_begin_reuses_one_operation(tmp_path: Path) -> None:
    ...
    assert (
        bool(a.get("must_not_repeat")) is False
        and bool(b.get("must_not_repeat")) is False
    )
